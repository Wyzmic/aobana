...
## 主な機能
- **自分だけのライブラリ** — 字幕（`.srt`）と電子書籍（`.epub`）を、指定した2つのフォルダから取り込みます。
...
1. [最新リリース](https://github.com/Wyzmic/aobana/releases/latest)から `Aobana-Setup-1.0.exe` をダウンロードします。
2. ダウンロードしたファイルを実行します。コード署名をしていないため、「WindowsによってPCが保護されました」という画面が出ることがあります。その場合は「詳細情報」をクリックし、「実行」を押してください。
3. 言語、インストール先、2つのメディアフォルダ（字幕用・書籍用）を指定します。表示されている新しいフォルダをそのまま使っても、既存のフォルダを選んでも構いません。字幕か書籍のどちらかしかない場合は、片方を空欄のままにしておけば、あとからライブラリタブで設定できます。続いて、インデックス（2つのデータベース）の保存先を選びます。既定は `%LOCALAPPDATA%\Aobana` で、露草のデータベースがすでにあるフォルダを指定することもできます。
   - セットアップをもう一度実行すると、現在のフォルダが入力された状態で同じ画面が開くので、あとから変更することもできます。
   - Python などの必要なものはすべて同梱されています。事前のインストールは不要で、オフラインでも動作します。
4. 必要に応じて「スタートメニューにショートカットを作成する」「デスクトップ上にアイコンを作成する」にチェックを入れます（どちらも既定ではオフです）。
5. **Aobana**（インストール先の `Aobana.exe`、またはショートカット）を起動すると、ブラウザで `http://127.0.0.1:5000/` が開きます。ポート 5000 がほかのプログラムに使われている場合は、セットアップが空いているポートを提案します。ポートはあとからライブラリタブで変更できます。

アンインストールでは、まずプログラム以外に何を削除するかを選べます。対象は設定とログ、`%LOCALAPPDATA%\Aobana` にあるインデックス、セットアップが提案した `ドキュメント\Aobana` のメディアフォルダで、それぞれのサイズも表示されます。すべて選べば何も残らず、再インストールに備えて残したいものだけ残すこともできます。ご自身で選んだフォルダ（メディアやインデックスの保存先）は削除されません。`data\ruby` に追加した行はプログラムのフォルダにあるため、一緒に削除されます。

## 使い方
1. **ファイルを置く** — 字幕は、字幕フォルダの中に作品ごとのフォルダを作って入れてください。フォルダ名がそのまま作品名になります。字幕フォルダの直下に置いた `.srt` は読み込まれないのでご注意ください。書籍は書籍フォルダ内のどこに置いても構いません。
   ```
   字幕フォルダ/
   ├─ 作品A/
   │   ├─ 作品A S01E01.srt
   ...
   └─ [著者名] 書名.epub
   ```
2. **インデックスを作成する** — ライブラリタブの「インデックス作成」を押します。ライブラリが大きい場合、初回は数分かかります。2回目以降は、変更のあったファイルだけを処理します。
3. **検索する**
...
1. **インストール** — 次のコマンドを Termux に貼り付けて実行し、ストレージへのアクセスを求められたら許可します。
   ...
   ```
   Termux 内に小さな Ubuntu 環境を用意し（形態素解析器の Sudachi に Android 版がないため）、そこに Python と、バージョンを固定したパッケージをインストールします。露草の動作に必要なファイルだけが `/storage/emulated/0/Aobana` に置かれ、Termux:Widget 用の **Aobana** ショートカットが追加されます。アップデートするときも、同じコマンドを実行するだけです。
2. **ライブラリを用意する** — パソコン版の露草で作成した `subs.db` と `epub.db` を、上記のフォルダにコピーします（いちばん速く、検索結果もパソコンと同じになります）。または、そのフォルダの `content/字幕` と `content/書籍` にファイルを入れ、ライブラリタブで「インデックス作成」を押します（スマートフォンでは時間がかかります）。
...
python launcher.py
```
ソースから実行した場合、メディアフォルダはコードと同じ場所の `content/字幕` と `content/書籍` になり、データベースもコードと同じ場所に作成されます。どちらのフォルダもライブラリタブから変更できます。

`requirements.txt` のバージョン指定は変えないでください。すべての文をどう区切ってインデックスするかは Sudachi の辞書で決まるため、辞書だけを更新すると検索結果が変わってしまいます。

Windows 用インストーラーを自分でビルドする手順は、`release/build.py` を参照してください。
...
予定している機能: **字幕形式の追加**。現在読み込めるのは `.srt` だけですが、次はアニメの字幕に多い `.ass` / `.ssa` に対応する予定です。
